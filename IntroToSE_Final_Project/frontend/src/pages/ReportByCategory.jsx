...
export default function ReportByCategory() {
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        const [byCatRes, txRes, walletsRes] = await Promise.all([
          api.reportByCategory({ type: 'expense' }),
          api.listTransactions(),
          api.listWallets(),
        ])

        const reportRows = byCatRes?.data || []
        const mappedRows = Array.isArray(reportRows)
          ? reportRows.map((r) => {
              ...
              return { key, name, total: Number(r?.total || 0), count: Number(r?.count || 0) }
            })
          : []
  ...
  const share = async () => {
    const payload = `Report by category (expenses)\nTotal: ${formatMoney(grand)}`
    try {
      if (navigator.share) {
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[320px_1fr]">
          <div className="rounded-2xl border border-border-dark bg-card-dark p-6">
            <div className="text-sm font-semibold text-text-secondary">Top category share</div>
              ...
              <div>
                ...
                {selected && (
                  <div className="mt-1 text-sm text-text-secondary">
                    Drill-down: <span className="font-semibold text-white">{selectedRow?.name}</span>
                  </div>
                )}
              </div>
            </div>
