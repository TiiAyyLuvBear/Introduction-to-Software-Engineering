...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Adjust based on typical axios response.data structure from api.js
      // transactionAPI.getAll returns response.data
      setTransactions(transactionsRes.transactions || (Array.isArray(transactionsRes) ? transactionsRes : []))

      // walletAPI.getUserWallets returns response.data
      // walletsRes could be { wallets: [] } or []
      setWallets(walletsRes.wallets || (Array.isArray(walletsRes) ? walletsRes : []))
            ...
            transactions.slice(0, 5).map(tx => (
              <div key={tx._id || tx.id} className="px-6 py-4 flex justify-between items-center hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-300 cursor-pointer transform hover:scale-[1.02]">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-md hover:shadow-lg transition-all duration-300 transform hover:rotate-12 ${tx.type === 'income' ? 'bg-gradient-to-br from-green-400 to-green-600' : 'bg-gradient-to-br from-red-400 to-red-600'
                    }`}>
                ...
                <div className="text-right">
                  ...
                  <div className={`text-lg font-bold ${tx.type === 'income' ? 'text-success' : 'text-danger'
                    }`}>
